...
// use strict to make code secure
...
var resizePizzas = function(size) {
  ...
  // Replaced document.querySelector with document.getElementById as it is fast.
  ...
  function changePizzaSizes(size) {
    // Replaced document.querySelector with document.getElementByClassName as it is fast.
    var randomPizzaContainer = document.getElementsByClassName("randomPizzaContainer");
    var containerLength = randomPizzaContainer.length;
    var dx = determineDx(randomPizzaContainer[0], size); // dx can be calculated outside the loop
    var newwidth = randomPizzaContainer[0].offsetWidth + dx + 'px'; // can be calculated outside loop
...
// pizzaDiv can be initialised outside the loop
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8,
      s = 256,
      dynamicHeight = Math.floor((window.innerHeight/s)*cols),
      elem,
      movingPizzas = document.getElementById('movingPizzas1');
     // Creating variables outside

  for (var i = 0; i < dynamicHeight; i++) {
